...
resource "oci_devops_build_pipeline_stage" "webui_build_pipeline_stage" {
  depends_on = [null_resource.pushcode,oci_objectstorage_bucket.bucket_webui]
  #Required
  ...
  build_pipeline_stage_predecessor_collection {
    ...
    items {
      #Required
      id = oci_devops_build_pipeline.webui.id
    }
  }
  ...
  build_spec_file = var.webui_buildspec
